...
hs <- org.Hs.eg.db
# loading data ----
load("~/workspace/neuroblastoma/RDSs/decon_eda_louis.RData")
...
row_annotation_table <- data.frame(Symbol = genes_to_plot$SYMBOL, row.names = genes_to_plot$ENSEMBL)
data_to_plot_subset <- vst_counts_removedBatchEffect_subset %>% filter(row.names(.) %in% genes_to_plot$ENSEMBL)
...
ssgsea_mes_adr_cellines <- GSVA::gsva(
  as.matrix(
    data.frame(vst_counts_removedBatchEffect) %>%
      dplyr::select(-matches("MNC_0[1-9].")) %>%
      dplyr::select(-matches("BMn_[6-9]."))
    ),
                                      signatures_list,
