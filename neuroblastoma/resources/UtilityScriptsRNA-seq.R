...
# Original function has a visual bug with scaling of x axis
# This is associated with: 
# scale_y_continuous(trans = .reverselog_trans(10))
# this is replaced with native function from the scale package
.dots_plot_fixed <- function(hyp_df,
                             top=20,
                             abrv=50,
  ...
  p <- ggplot(df, aes(x = label.abrv, 
    ...
    labs(title = title, 
         y = color.label,
         color = color.label) + 
    scale_color_continuous(low = "#E53935", 
                           high = "#114357",
