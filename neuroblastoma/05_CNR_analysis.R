...
exp_id <- "20230828"
res_dir <- file.path("/home/rstudio/workspace/neuroblastoma/results", exp_id)
...
### Plotting general stats of the run
# Plot peaks count per condition
ggplot(samples) +
  aes(x = Factor, y = Peaks_count, fill = Condition) +
  geom_boxplot() +
...
###########
# run only once
# upload consensus peaks for every protein
for (target_prot in unique(samples$Factor)) {
  if (!file.exists(file.path(res_dir, paste0("consensus_peaks/dba_", target_prot, "_not_annotated.RData")))) {
    if (target_prot == "FRA1") {
      next
    }
    print(c("Current protein is ", target_prot))
    dfbobj <- dba(sampleSheet = samples %>% filter(., Factor == target_prot))
    dfbobj <- dba.count(dfbobj)
    ...
    conssensus_peaks <- as.data.frame(dba.peakset(dfbobj, bRetrieve = T))[, 1:3]
    conssensus_peaks <- cbind(
      row.names(conssensus_peaks),
      conssensus_peaks,
      "."
    )
    conssensus_peaks$seqnames <- paste0("chr", conssensus_peaks$seqnames)
    print(dim(conssensus_peaks))
    write.table(conssensus_peaks,
      file = file.path(res_dir, paste0("consensus_peaks/", target_prot, "_consenus_peaks.bed")),
      sep = "\t",
      row.names = F,
      col.names = F,
      quote = F
    )
    dba.save(dfbobj, dir = file.path(res_dir, paste0("consensus_peaks/")), file = paste0(target_prot, "_not_annotated"))
  }
}

# then these consensus peaks are processed by HOMER annotatePeaks.pl (outside R)
# The script is here:
# ~/workspace/neuroblastoma/results/20230828/consensus_peaks
# now attach the annotation to the DiffBind objects or load the object
DBobj_list <- list()
for (target_prot in unique(samples$Factor)) {
  if (target_prot == "FRA1") {
    next
  }
  if (file.exists(file.path(res_dir, paste0("consensus_peaks/dba_", target_prot, ".RData")))) {
    print(paste("File", target_prot, "exists, loading ..."))
    fl_path <- file.path(res_dir, paste0("consensus_peaks/dba_", target_prot))
    ...
    fl_path <- file.path(res_dir, paste0("consensus_peaks/dba_", target_prot, "_not_annotated"))
    dfbobj <- dba.load(fl_path, dir = "", pre = "")
    mcols_data <- mcols(dfbobj$DESeq2$DEdata)
    mcols_data <- cbind(row.names(mcols_data), mcols_data)
    annotation_file <- read.csv2(file = file.path(res_dir, "consensus_peaks", 
                                                  paste0(target_prot, "_consenus_peaks.bed_output.txt")
                                                  ), 
                                 sep = "\t")
    mcols(dfbobj$DESeq2$DEdata) <- merge.data.frame(mcols_data, 
                                                    annotation_file, 
                                                    by.x = 1, 
                                                    by.y = 1, 
                                                    sort = F)
...
# Create a list of real consensus peaks
...
for_hist <- as.data.frame(ol$overlappingPeaks$`TAZ_Peaks///YAP_Peaks`$shortestDistance)
colnames(for_hist) <- "shortestDistance"
ggplot(for_hist) +
  aes(
    x = shortestDistance
  ) +
...
for_corr_plot <- data.frame(
  ...
  filter(TAZ < 0.2 & YAP < 0.2)
for_corr_plot <- for_corr_plot
...
# Plot the profile for different proteins and chr. tags
# needs to be fixed
# library(TxDb.Hsapiens.UCSC.hg38.knownGene)
# promoter <- ChIPseeker::getPromoters(TxDb = TxDb.Hsapiens.UCSC.hg38.knownGene, upstream = 3000, downstream = 3000)
# tmp <- GRanges(as.data.frame(DBobj_list_cons$YAP)[, 1:3])
# tagMatrix <- ChIPseeker::getTagMatrix(tmp, windows = promoter)
...
write.table(
  x = mes_adrn_gene_list,
  file = "~/workspace/neuroblastoma/resources/mes_adrn_GS_frm_RNA_seq2.tsv",
  sep = "\t",
  row.names = F,
  quote = F
)
...
#our_rna_seq_terms <- "/home/rstudio/workspace/neuroblastoma/resources/mes_adrn_GS_from_RNA_seq.tsv"
our_rna_seq_terms <- "~/workspace/neuroblastoma/resources/mes_adrn_GS_frm_RNA_seq2.tsv"
Grngn_mes_adrn_gene_list <- "/home/rstudio/workspace/neuroblastoma/resources/mes_adrn_genes.bed"
...
for(TF_name in names(DBobj_list)){
  ...
  if (nrow(p_MES_up) > 0) {
    ...
    chipEnrichAndExport(peaks = p_MES_up,
                        ...
                        TF_name = TF_name, 
                        res_dir = res_dir, 
                        genesets = Grngn_mes_adrn_gene_list, 
                        genesets_name = "Groen_",
                        locusdef = locusdef
    )
  }
...
# Split plots
pdf(file = file.path(res_dir, "plots/TFs_Hmods_MES_ADRN_our_data.pdf"))
...
for(peak_names in names(YAP_TAZ_JUN_peaks_list)){
  ...
  promoter_ol <- ChIPpeakAnno::findOverlapsOfPeaks(promoter, H3k27ac_Peaks)
  promoter <- promoter_ol$mergedPeaks
  H3k27ac_Peaks_no_prom <- promoter_ol$peaklist$H3k27ac_Peaks
  
  Enh_peaks_ac27_me1_ol <- ChIPpeakAnno::findOverlapsOfPeaks(H3k27ac_Peaks_no_prom, H3k4me1_Peaks)
  Enh_peaks_ac27 <- Enh_peaks_ac27_me1_ol$peaklist$H3k27ac_Peaks_no_prom
  Enh_peaks_me1 <- Enh_peaks_ac27_me1_ol$peaklist$H3k4me1_Peaks
  Enh_peaks_ac27_me1 <- Enh_peaks_ac27_me1_ol$peaklist$`H3k27ac_Peaks_no_prom///H3k4me1_Peaks`
  
  ol <- ChIPpeakAnno::findOverlapsOfPeaks(peaks, promoter)
  promoters_percent <- length(peaks) - length(ol$peaklist$peaks)
  
  ol <- ChIPpeakAnno::findOverlapsOfPeaks(peaks, Enh_peaks_ac27)
  Enh_peaks_ac27_percent <- length(peaks) - length(ol$peaklist$peaks)
  
  ol <- ChIPpeakAnno::findOverlapsOfPeaks(peaks,  Enh_peaks_me1)
  Enh_peaks_me1_percent <- length(peaks) - length(ol$peaklist$peaks)
  
  ol <- ChIPpeakAnno::findOverlapsOfPeaks(peaks, Enh_peaks_ac27_me1)
  Enh_peaks_ac27_me1_percent <- length(peaks) - length(ol$peaklist$peaks)
  
  unchar <- length(peaks) - Enh_peaks_ac27_me1_percent - Enh_peaks_me1_percent - Enh_peaks_ac27_percent - promoters_percent
  ...
  piechart <- rbind(piechart,
                    tibble(
                      ...
                      number_of_peaks = c(
                        unchar,
                        Enh_peaks_ac27_me1_percent,
                        Enh_peaks_ac27_percent,
                        Enh_peaks_me1_percent,
                        promoters_percent
                      )
                    )
  )
...
for (target_prot in unique(samples$Factor)) {
  ...
  fl_path <- paste0("home/rstudio/workspace/neuroblastoma/results/20230828/consensus_peaks/dba_", target_prot)
  dfbobj <- dba.load(fl_path, dir = "", pre = "")
  ...
  write.table(
    ...
    file = paste0("neuroblastoma/results/20230828/consensus_peaks/", target_prot, "_consenus_peaks_UP.bed"),
    sep = "\t",
    row.names = T,
    col.names = F,
    quote = F
  ...
  write.table(
    as.data.frame(repDWN)[, 1:3] %>%
      dplyr::select(seqnames, start, end) %>%
      mutate(seqnames = paste0("chr", seqnames), chain = "."),
    file = paste0("neuroblastoma/results/20230828/consensus_peaks/", target_prot, "_consenus_peaks_DWN.bed"),
    sep = "\t",
    row.names = T,
    col.names = F,
    quote = F
...
#############
# Peak profiles
pdf(file = file.path(res_dir, "plots/h3k27ac-h3k4me1-jun-taz-yap.pdf"), width = 12, height = 6)
