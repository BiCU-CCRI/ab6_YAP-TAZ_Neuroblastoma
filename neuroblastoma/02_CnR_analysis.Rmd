...
debug(chipEnrichAndExport)
debug(subsetPeaksInSamples)
...
for (target_prot in names(DBobj_list)) {
  ...
  write.table(
    ...
    file = paste0("~/workspace/neuroblastoma/results/CnR/", target_prot, "_consenus_peaks_UP.bed"),
    sep = "\t",
    row.names = T,
    col.names = F,
    quote = F
  ...
  write.table(
    as.data.frame(repDWN)[, 1:3] %>%
      dplyr::select(seqnames, start, end) %>%
      mutate(seqnames = paste0("chr", seqnames), chain = "."),
    file = paste0("~/workspace/neuroblastoma/results/CnR/", target_prot, "_consenus_peaks_DWN.bed"),
    sep = "\t",
    row.names = T,
    col.names = F,
    quote = F
