...
deg_dir <- "~/neuroblastoma/results/ATAC-seq/ATAC-seq_MES_vs_ADR"
```
...
path_to_the_ATAC_counts_data <- file.path(tmp_dir, "legacy_all_filtered.RDS")
path_annnotation_ATAC_data <- file.path(tmp_dir, "legacy_consensus_regions_annotation.RDS")
...
mcols(ATAC_dds)$gene_category_our_RNAseq <- case_when(
  mcols(ATAC_dds)$homer_Gene.Name %in% RNA_SEQ_data[RNA_SEQ_data$Term == "ADR", 1] ~ "g_ADR",
...
heatmap_counts_ADRN_spec_genes <- heatmap_counts[rownames(heatmap_counts) %in% 
                                                   ATAC_signif$peak_id[ATAC_signif$gene_category_our_RNAseq == "g_ADR"],]
...
ATAC_genome_ADR <- read.delim2("~/neuroblastoma/results/ATAC-seq/ATAC-seq_MES_vs_ADR/ATAC_genome_states_dataframe_ADR.csv", 
                               sep="\t", 
                               header=T)[-c(1:14),]
ATAC_genome_MES <- read.delim2("~/neuroblastoma/results/ATAC-seq/ATAC-seq_MES_vs_ADR/ATAC_genome_states_dataframe_MES.csv",
                               sep="\t",
                               header=T)[-c(1:14),]
...
heatmap_counts_ADRN_spec_genes <- heatmap_counts[rownames(heatmap_counts) %in% ATAC_signif$peak_id[ATAC_signif$gene_category_our_RNAseq == "g_ADR"],]
...
res_dir <- "~/neuroblastoma/results/ATAC-seq/ATAC-seq_MES_vs_ADR/"
