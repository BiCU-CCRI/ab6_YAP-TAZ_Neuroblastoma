...
ggsave(
  filename = paste0(PATHS$results_dir, "24H_vs_control_volcano_plot.png"),
  plot = volcano_plot,
...
ggsave(
  filename = paste0(PATHS$results_dir, "ssgsea_mes_adr_ncc_noradr_heatmap_OUR_RNASEQ.pdf"),
...
ssgsea_mes_adr_ncc_noradr_heatmap_MES <- pheatmap::pheatmap(ssgsea_mes_adr_cellines_MES,
                                                        ...
                                                        show_colnames = TRUE)
ggsave(
  filename = paste0(PATHS$results_dir, "ssgsea_mes_adr_ncc_noradr_heatmap_OUR_RNASEQ_MES.pdf"),
...
ssgsea_mes_adr_ncc_noradr_heatmap_ADR <- pheatmap::pheatmap(ssgsea_mes_adr_cellines_ADR,
                                                            ...
                                                            show_colnames = TRUE)
ggsave(
  filename = paste0(PATHS$results_dir, "ssgsea_mes_adr_ncc_noradr_heatmap_OUR_RNASEQ_ADR.pdf"),
...
for (cell_line_name in c("CM", "SH")){
  ...
  for(comparison_name in resultsNames(dds_cell_line_subset)[2:3]){
    ...
    writeData(XLSX_OUT, x = deg_results$results_all, sheet = "results_all")
    saveWorkbook(XLSX_OUT, 
                 file.path(PATHS$results_dir, paste0(comparison_name, "_", ".xlsx")),
                 overwrite = TRUE)
    ...
    ggsave(
      filename = paste0(PATHS$results_dir, paste0(comparison_name, "_more_highlited_genes.pdf")),
      plot = volcano_plot,
      width = 20, height = 20, units = "cm"
    )
    print(volcano_plot)
    ...
    volcano_plot <- plotVolcano(
      ...
      scale_y_continuous(limits = c(-5, 320))
    ggsave(
      filename = paste0(PATHS$results_dir, paste0(comparison_name, "_less_highlited_genes.pdf")),
      plot = volcano_plot,
      width = 20, height = 20, units = "cm"
    )
    print(volcano_plot)
