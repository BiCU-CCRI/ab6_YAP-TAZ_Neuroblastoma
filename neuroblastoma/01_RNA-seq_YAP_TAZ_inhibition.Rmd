...
## 24h vs Control Analysis

```{r analysis_24h_vs_control}
message("Running 24h vs control analysis...")

# Check available contrasts
message("Available contrasts: ", paste(resultsNames(dds), collapse = ", "))

# Generate differential expression results
deg_results_24h <- generateResults(
  dds_object = dds,
...
# Validate results
stat_validation_24h <- validate_statistical_results(
  deg_results_24h$results_all, 
  config$statistical$padj_cutoff
)
...
# Create summary
summary_24h <- generate_analysis_summary(deg_results_24h)
summary_df <- data.frame(
  Metric = names(summary_24h),
  Value = unlist(summary_24h),
  stringsAsFactors = FALSE
)

writeData(xlsx_out, "Summary", summary_df)
writeData(xlsx_out, "Significant_DEGs", deg_results_24h$results_signif)
writeData(xlsx_out, "DE_Details", deg_results_24h$de_details)
writeData(xlsx_out, "All_Results", deg_results_24h$results_all)
...
volcano_plot_24h <- create_enhanced_volcano(
  ...
  genes_highlight = c("VIM", "YAP1", "WWTR1", "JUN", "FOSL1", "FOSL2", "PHOX2B", "HAND2", "GATA3"),
...
safe_save_plot(
  volcano_plot_24h,
  file.path(PATHS$results_dir, "plots", "volcano_24h_vs_control.png"),
...
print(volcano_plot_24h)

# Prepare data for heatmaps
metadata_heatmap <- as.data.frame(colData(dds))
dds_signif <- deg_results_24h$results_signif
