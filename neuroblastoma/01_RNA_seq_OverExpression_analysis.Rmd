...
PATHS <- list(
  ...
  results_dir = here::here("neuroblastoma", "results", "RNA-seq_OE"),
...
```{r load_data}
message("Loading overexpression and control datasets...")

# Load the new DESeq2 file
load(file.path(PATHS$data_dir, "rnaseq_yap_taz_OE.RData"))
new_dds_assay <- as.data.frame(counts(dds))
new_dds_assay$names_of_rows <- row.names(new_dds_assay)
rm(dds)

# Load wildtype control data
wt_dds <- readRDS(file.path(PATHS$data_dir, "rnaseq_deseq_global_deseq_data_set.rds"))
wt_dds_assay <- as.data.frame(counts(wt_dds))
wt_dds_assay$names_of_rows <- row.names(wt_dds_assay)
rm(wt_dds)

# Combine datasets with data cleaning
combined_counts <- merge.data.frame(
  new_dds_assay, 
  wt_dds_assay, 
  by = "names_of_rows"
) %>%
...
message("Combined dataset dimensions: ", nrow(combined_counts), " genes x ", ncol(combined_counts), " samples")

# Create comprehensive sample metadata
combined_counts_col_data <- tibble(sample_name = colnames(combined_counts)) %>%
  mutate(
    cell_line = case_when(
      str_detect(sample_name, "CM") ~ "CM",
      str_detect(sample_name, "SH") ~ "SH",
      TRUE ~ "Unknown"
    ),
    exp_type = case_when(
